# hubspot-conversations-cli

boru-driven command-line client **and** interactive REPL for the HubspotConversations
...
```sh
# 1. Build a native binary (-> dist/<os>-<arch>/hubspot-conversations-cli)
make build

# 2. See usage (words, entities, env vars)
./hubspot-conversations-cli --help
...
HUBSPOT_CONVERSATIONS_BASE=https://api.example.com ./hubspot-conversations-cli --help

# 6. No arguments -> interactive REPL
./hubspot-conversations-cli
hubspot-conversations> /help
hubspot-conversations> /quit
```
...
1. **Build the binary.** From this `go-cli/` directory:
   ...
   ```sh
   make build          # -> dist/<os>-<arch>/hubspot-conversations-cli
   ```
...
3. **Run a query.** Evaluate an boru expression against the API (or run with no
   ...
   ```sh
   ./dist/*/hubspot-conversations-cli --help
   ```
...
Run with no arguments to open a REPL (prompt `hubspot-conversations>`). Each line is
...
- `<entity>` is a bareword, auto-quoted as an boru atom (e.g. `channel`).
...
| Target | Result |
|--------|--------|
| `make build` | Native binary at `dist/<os>-<arch>/hubspot-conversations-cli`. |
...
(`./hubspot-conversations-cli <expr>`) and interactively (the REPL), and expressions compose the

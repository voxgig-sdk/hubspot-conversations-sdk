// Command HubspotConversations-cli is a CLI + REPL for the HubspotConversations SDK,
...
func usage(out io.Writer) {
	...
	fmt.Fprintln(out, "Env:      HUBSPOT_CONVERSATIONS_APIKEY (api key), HUBSPOT_CONVERSATIONS_BASE (base url override)")
...
func run(args []string, in io.Reader, out, errOut io.Writer) int {
	...
	// Configure from the environment: HUBSPOT_CONVERSATIONS_APIKEY carries the API key and
	// HUBSPOT_CONVERSATIONS_BASE optionally overrides the API base URL (e.g. production).
	// Both injectable by a secrets vault. Unset -> nil config defaults.
	var opts map[string]any
	if apikey := os.Getenv("HUBSPOT_CONVERSATIONS_APIKEY"); apikey != "" {
		opts = map[string]any{"apikey": apikey}
	}
	if base := os.Getenv("HUBSPOT_CONVERSATIONS_BASE"); base != "" {
		if opts == nil {
			opts = map[string]any{}
		}
		opts["base"] = base
	}
	client := sdk.NewHubspotConversationsSDK(opts)

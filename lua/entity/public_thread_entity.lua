...
local PublicThreadEntity = {}
PublicThreadEntity.__index = PublicThreadEntity


function PublicThreadEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, PublicThreadEntity)
  self._name = "public_thread"
  self._client = client
...
function PublicThreadEntity:get_name()
  return self._name
end


function PublicThreadEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return PublicThreadEntity.new(self._client, opts)
end
...
function PublicThreadEntity:mark_deleted()
  self._deleted = true
end


function PublicThreadEntity:deleted()
  return true == self._deleted
end


function PublicThreadEntity:data_set(args)
  if args ~= nil then
...
function PublicThreadEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function PublicThreadEntity:match_set(args)
  if args ~= nil then
...
function PublicThreadEntity:match_get()
...
function PublicThreadEntity:stream(action, args, callopts)
...
---@param reqmatch PublicThreadRemoveMatch
---@param ctrl? table
---@return PublicThread
---@return string? err
function PublicThreadEntity:remove(reqmatch, ctrl)
  local utility = self._utility
...
function PublicThreadEntity:_run_op(ctx, post_done)
  local utility = self._utility

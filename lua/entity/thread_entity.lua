...
local ThreadEntity = {}
ThreadEntity.__index = ThreadEntity


function ThreadEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, ThreadEntity)
  self._name = "thread"
  self._client = client
...
function ThreadEntity:get_name()
  return self._name
end


function ThreadEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return ThreadEntity.new(self._client, opts)
end
...
function ThreadEntity:mark_deleted()
  self._deleted = true
end


function ThreadEntity:deleted()
  return true == self._deleted
end


function ThreadEntity:data_set(args)
  if args ~= nil then
...
function ThreadEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function ThreadEntity:match_set(args)
  if args ~= nil then
...
function ThreadEntity:match_get()
...
function ThreadEntity:stream(action, args, callopts)
...
---@param reqmatch ThreadRemoveMatch
---@param ctrl? table
---@return Thread
---@return string? err
function ThreadEntity:remove(reqmatch, ctrl)
  local utility = self._utility
...
function ThreadEntity:_run_op(ctx, post_done)
  local utility = self._utility

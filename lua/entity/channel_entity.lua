...
local ChannelEntity = {}
ChannelEntity.__index = ChannelEntity


function ChannelEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, ChannelEntity)
  self._name = "channel"
  self._client = client
...
function ChannelEntity:get_name()
  return self._name
end


function ChannelEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return ChannelEntity.new(self._client, opts)
end
...
function ChannelEntity:mark_deleted()
  self._deleted = true
end


function ChannelEntity:deleted()
  return true == self._deleted
end


function ChannelEntity:data_set(args)
  if args ~= nil then
...
function ChannelEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function ChannelEntity:match_set(args)
  if args ~= nil then
...
function ChannelEntity:match_get()
...
function ChannelEntity:stream(action, args, callopts)
...
function ChannelEntity:_run_op(ctx, post_done)
  local utility = self._utility

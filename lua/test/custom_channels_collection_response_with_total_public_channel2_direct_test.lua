...
describe("CustomChannelsCollectionResponseWithTotalPublicChannel2Direct", function()
  it("should direct-list-custom_channels_collection_response_with_total_public_channel2", function()
    local setup = custom_channels_collection_response_with_total_public_channel2_direct_setup({
      { id = "direct01" },
      { id = "direct02" },
    })
    local _should_skip, _reason = runner.is_control_skipped("direct", "direct-list-custom_channels_collection_response_with_total_public_channel2", setup.live and "live" or "unit")
    if _should_skip then
      pending(_reason or "skipped via sdk-test-control.json")
      return
    end
    if setup.live then
      for _, _live_key in ipairs({"channel01"}) do
...
function custom_channels_collection_response_with_total_public_channel2_direct_setup(mockres)

...
describe("ChannelEntity", function()
  ...
  it("should run basic flow", function()
    ...
    if setup.synthetic_only then
      pending("live entity test uses synthetic IDs from fixture — set HUBSPOT_CONVERSATIONS_TEST_CHANNEL_ENTID JSON to run live")
      return
    end
    local client = setup.client

    -- Bootstrap entity data from existing test data.
    local channel_ref01_data_raw = vs.items(helpers.to_map(
      vs.getpath(setup.data, "existing.channel")))
    local channel_ref01_data = nil
    if #channel_ref01_data_raw > 0 then
      channel_ref01_data = helpers.to_map(channel_ref01_data_raw[1][2])
    end
...
function channel_basic_setup(extra)
  ...
  local entid_env_raw = os.getenv("HUBSPOT_CONVERSATIONS_TEST_CHANNEL_ENTID")
  ...
  local idmap_resolved = helpers.to_map(
    env["HUBSPOT_CONVERSATIONS_TEST_CHANNEL_ENTID"])

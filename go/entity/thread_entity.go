...
func NewThreadEntity(client *core.HubspotConversationsSDK, entopts map[string]any) *ThreadEntity {
	if entopts == nil {
		entopts = map[string]any{}
	}
...
func (e *ThreadEntity) GetName() string { return e.name }
...
func (e *ThreadEntity) MarkDeleted() {
	e.deleted = true
}


// Deleted reports whether a successful Remove has resolved on this instance.
func (e *ThreadEntity) Deleted() bool {
	return e.deleted
}


func (e *ThreadEntity) Make() core.Entity {
	opts := map[string]any{}
	for k, v := range e.entopts {
		opts[k] = v
	}
	return NewThreadEntity(e.client, opts)
}

func (e *ThreadEntity) Data(args ...any) any {
...
func (e *ThreadEntity) Match(args ...any) any {
	if len(args) > 0 && args[0] != nil {
...
// argument it returns the current data as an Thread; with an argument it
// sets the data and returns the stored value. It delegates to the untyped Data
// (identical runtime) and converts at the typed boundary.
func (e *ThreadEntity) DataTyped(data ...Thread) Thread {
	if len(data) > 0 {
		return typedFrom[Thread](e.Data(asMap(data[0])))
	}
	return typedFrom[Thread](e.Data())
}

// MatchTyped mirrors DataTyped for the entity's match filter. The match is a
// partial of the entity, so it round-trips through Thread (all fields
// optional at the wire level).
func (e *ThreadEntity) MatchTyped(match ...Thread) Thread {
	if len(match) > 0 {
		return typedFrom[Thread](e.Match(asMap(match[0])))
	}
	return typedFrom[Thread](e.Match())
}
...
func (e *ThreadEntity) Stream(action string, args map[string]any, callopts map[string]any) <-chan any {
	out := make(chan any)
...
func (e *ThreadEntity) Load(_ map[string]any, _ map[string]any) (any, error) {
	return core.UnsupportedOp("load", e.name)
}


func (e *ThreadEntity) List(_ map[string]any, _ map[string]any) (any, error) {
	return core.UnsupportedOp("list", e.name)
}


func (e *ThreadEntity) Create(_ map[string]any, _ map[string]any) (any, error) {
	return core.UnsupportedOp("create", e.name)
}


func (e *ThreadEntity) Update(_ map[string]any, _ map[string]any) (any, error) {
	return core.UnsupportedOp("update", e.name)
}



func (e *ThreadEntity) Remove(reqmatch map[string]any, ctrl map[string]any) (any, error) {
	utility := e.utility
...
// ThreadRemoveMatch and returns an Thread. It delegates to the untyped
// Remove (identical runtime) and converts at the typed boundary.
func (e *ThreadEntity) RemoveTyped(reqmatch ThreadRemoveMatch, ctrl map[string]any) (Thread, error) {
	res, err := e.Remove(asMap(reqmatch), ctrl)
	if err != nil {
		return Thread{}, err
	}
	return typedFrom[Thread](res), nil
}



func (e *ThreadEntity) runOp(ctx *core.Context, postDone func()) (any, error) {
	utility := e.utility

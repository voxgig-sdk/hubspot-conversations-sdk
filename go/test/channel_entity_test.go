...
func TestChannelEntity(t *testing.T) {
	...
	t.Run("basic", func(t *testing.T) {
		...
		if setup.syntheticOnly {
			t.Skip("live entity test uses synthetic IDs from fixture — set HUBSPOT_CONVERSATIONS_TEST_CHANNEL_ENTID JSON to run live")
			return
		}
		// Bootstrap entity data from existing test data (no create step in flow).
		channelRef01DataRaw := vs.Items(core.ToMapAny(vs.GetPath(setup.data, "existing.channel")))
		var channelRef01Data map[string]any
		if len(channelRef01DataRaw) > 0 {
			channelRef01Data = core.ToMapAny(channelRef01DataRaw[0][1])
		}
...
func channelBasicSetup(extra map[string]any) *entityTestSetup {
	...
	entidEnvRaw := os.Getenv("HUBSPOT_CONVERSATIONS_TEST_CHANNEL_ENTID")
	...
	idmapResolved := core.ToMapAny(env["HUBSPOT_CONVERSATIONS_TEST_CHANNEL_ENTID"])

...
func TestThreadEntity(t *testing.T) {
	...
	t.Run("basic", func(t *testing.T) {
		...
		if setup.syntheticOnly {
			t.Skip("live entity test uses synthetic IDs from fixture — set HUBSPOT_CONVERSATIONS_TEST_THREAD_ENTID JSON to run live")
			return
		}
...
func threadBasicSetup(extra map[string]any) *entityTestSetup {
	...
	idmap, _ := vs.Transform(
		[]any{"thread01", "thread02", "thread03"},
		map[string]any{
	...
	entidEnvRaw := os.Getenv("HUBSPOT_CONVERSATIONS_TEST_THREAD_ENTID")
	...
	idmapResolved := core.ToMapAny(env["HUBSPOT_CONVERSATIONS_TEST_THREAD_ENTID"])

...
from projectname_sdk import HubspotConversationsSDK
from projectname_sdk.core.error import HubspotConversationsError
from projectname_sdk.core.result import HubspotConversationsResult
from projectname_sdk.core.response import HubspotConversationsResponse
from projectname_sdk.core.spec import HubspotConversationsSpec
from projectname_sdk.feature.base_feature import HubspotConversationsBaseFeature


def _client():
    return HubspotConversationsSDK.test(None, None)
...
def _full_spec():
    return HubspotConversationsSpec({
        "base": "http://h",
        "prefix": "",
        "suffix": "",
        "path": "a",
...
def _resp(status, data=None, headers=None):
    ...
    return HubspotConversationsResponse({
        "status": status,
...
class TestMakePointAndMakeSpec:
    ...
    def test_make_spec_short_circuits_a_feature_supplied_spec(self):
        client = _client()
        ctx = _ctx(client, opname="load")
        preset = HubspotConversationsSpec({"method": "GET"})
        ctx.out["spec"] = preset
...
class TestMakeResponse:
    ...
    def test_guards_missing_spec_response_result(self):
        ...
        ctx.result = HubspotConversationsResult({})
...
class TestMakeResult:
    ...
    def test_list_op_wraps_resdata_into_entity_instances(self):
        ...
        ctx.entity = entity
        ctx.spec = _full_spec()
        ctx.result = HubspotConversationsResult({"resdata": [{"a": 1}, {"a": 2}]})
    ...
    def test_an_empty_list_yields_an_empty_resdata_array(self):
        ...
        ctx.spec = _full_spec()
        ctx.result = HubspotConversationsResult({"resdata": []})
        result, err = client._utility.make_result(ctx)
        assert err is None
    ...
    def test_short_circuits_on_a_preset_result(self):
        client = _client()
        ctx = _ctx(client)
        preset = HubspotConversationsResult({"ok": True})
...
class TestMakeRequest:
    ...
    def test_short_circuits_a_feature_supplied_request(self):
        client = _client()
        ctx = _ctx(client)
        preset = HubspotConversationsResponse({"status": 201, "statusText": "OK"})
...
class TestMakeErrorAndDone:
    ...
    def test_done_returns_resdata_on_success(self):
        client = _client()
        ctx = _ctx(client)
        ctx.result = HubspotConversationsResult({"ok": True, "resdata": 42})
        assert client._utility.done(ctx) == 42

    def test_done_raises_the_error_when_not_ok(self):
        client = _client()
        ctx = _ctx(client)
        ctx.result = HubspotConversationsResult({"ok": False})
        with pytest.raises(HubspotConversationsError):
            client._utility.done(ctx)

    def test_done_cleans_ctrl_explain_on_success(self):
        client = _client()
        ctx = _ctx(client, ctrl={"explain": {"result": {"err": "x"}}})
        ctx.result = HubspotConversationsResult({"ok": True, "resdata": 7})
    ...
    def test_make_error_returns_resdata_when_throw_is_disabled(self):
        ...
        ctx.result = HubspotConversationsResult({"ok": False, "resdata": "fallback"})
...
class TestFeatureOrdering:
    ...
    def _named_feature(self, name):
        feature = HubspotConversationsBaseFeature()
        feature.name = name
        return feature
...
class TestPrepareAuth:
    ...
    def _auth_ctx(self, client, options, headers):
        ...
        ctx.spec = None if headers is None else HubspotConversationsSpec(
            {"headers": headers})
        return ctx
...
class TestResultHelpers:
    ...
    def test_result_headers_with_no_headers_yields_an_empty_map(self):
        client = _client()
        ctx = _ctx(client)
        ctx.response = HubspotConversationsResponse({"status": 200})
        ctx.result = HubspotConversationsResult({})

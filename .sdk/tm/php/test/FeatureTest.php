...
// HubspotConversations SDK feature test
//
...
class FtCtrl extends HubspotConversationsControl
{
...
class FtClient
{
    ...
    private HubspotConversationsUtility $utility;

    public function __construct(HubspotConversationsUtility $utility, array $options)
    {
    ...
    public function get_utility(): HubspotConversationsUtility
    {
        return $this->utility;
    }
}
...
class FtRecorder
{
    ...
    {
        return function (HubspotConversationsContext $_ctx, string $url, array $fetchdef): array {
...
class FtHarness
{
    ...
    public HubspotConversationsContext $rootctx;
    ...
    public function __construct(
        ...
        $utility->fetcher = $server ?? function (HubspotConversationsContext $_ctx, string $_url, array $fetchdef): array {
        ...
        foreach ($features as $fspec) {
            ...
            $f = HubspotConversationsConfig::make_feature($name);
    ...
    private static function buildUrl(HubspotConversationsSpec $spec): string
    {
        $keys = [];
    ...
    public function op(array $o = []): array
    {
        ...
        if (($ctx->out['point'] ?? null) instanceof HubspotConversationsError) {
        ...
        if (is_array($fetched)) {
            $ctx->response = new HubspotConversationsResponse($fetched);
        }
    ...
    private function fail(HubspotConversationsContext $ctx, mixed $err): array
    {
...
class FeatureTest extends TestCase
{
    ...
    public function test_retry_retries_a_transport_error_until_exhausted(): void
    {
        ...
        $clock = new FtClock();
        $n = 0;
        $server = function (HubspotConversationsContext $ctx, string $_url, array $_fd) use (&$n): array {
            $n++;

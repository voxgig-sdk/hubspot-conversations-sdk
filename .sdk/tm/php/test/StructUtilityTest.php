...
// HubspotConversations SDK struct utility test
//
...
class StructUtilityTest extends TestCase
{
    ...
    {
        if (self::$run === null) {
            $runner = HubspotConversationsOmni::makeStructRunner(
    ...
    {
        return HubspotConversationsOmni::objmodel(HubspotConversationsOmni::argval($val));
    }

    /** A single-case fixture value on the comparison side. */
    private static function outval($val)
    {
        return HubspotConversationsOmni::unmark($val);
    }

    /** A result normalised to the comparison model. */
    private static function resval($val)
    {
        return HubspotConversationsOmni::structfix($val, Struct::undef());
    }
        ...
        $this->runsection(
            ...
            function ($input) {
                ...
                if (is_object($store) && $haspath && Struct::undef() !== $val) {
                    ...
                    if (json_encode(HubspotConversationsOmni::structfix($now, Struct::undef()))
                        !== json_encode(HubspotConversationsOmni::structfix($val, Struct::undef()))) {

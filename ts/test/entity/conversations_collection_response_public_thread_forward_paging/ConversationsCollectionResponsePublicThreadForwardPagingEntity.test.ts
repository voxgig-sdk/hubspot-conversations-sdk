...
describe('ConversationsCollectionResponsePublicThreadForwardPagingEntity', async () => {
  ...
  test('basic', async (t) => {
    ...
    for (const op of ['list']) {
      if (!live && maybeSkipControl(t, 'entityOp', 'conversations_collection_response_public_thread_forward_paging.' + op, live)) return
    }
    ...
    let conversations_collection_response_public_thread_forward_paging_ref01_data = Object.values(setup.data.existing.conversations_collection_response_public_thread_forward_paging)[0] as any
    ...
    const conversations_collection_response_public_thread_forward_paging_ref01_match: any = {}

    const conversations_collection_response_public_thread_forward_paging_ref01_list = (await conversations_collection_response_public_thread_forward_paging_ref01_ent.list(conversations_collection_response_public_thread_forward_paging_ref01_match)).map((e: any) => e.data())

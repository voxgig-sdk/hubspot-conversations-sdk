...
describe('ConversationsCollectionResponsePublicMessageForwardPagingDirect', async () => {
  ...
  test('direct-list-conversations_collection_response_public_message_forward_paging', async (t: any) => {
    if (liveScenariosActive()) { t.skip('Covered by live operation scenarios'); return }
    const setup = directSetup([{ id: 'direct01' }, { id: 'direct02' }])
    if (maybeSkipControl(t, 'direct', 'direct-list-conversations_collection_response_public_message_forward_paging', setup.live)) return
    if (skipIfMissingIds(t, setup, ["thread01"])) return
...
function directSetup(mockres?: any) {
  ...
  if (live) {
    ...
    let idmap: any = env['HUBSPOT_CONVERSATIONS_TEST_CONVERSATIONS_COLLECTION_RESPONSE_PUBLIC_MESSAGE_FORWARD_PAGING_ENTID']

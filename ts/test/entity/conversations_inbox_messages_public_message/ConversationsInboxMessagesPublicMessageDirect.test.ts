...
describe('ConversationsInboxMessagesPublicMessageDirect', async () => {
  ...
  test('direct-load-conversations_inbox_messages_public_message', async (t: any) => {
    if (liveScenariosActive()) { t.skip('Covered by live operation scenarios'); return }
    const setup = directSetup({ id: 'direct01' })
    if (maybeSkipControl(t, 'direct', 'direct-load-conversations_inbox_messages_public_message', setup.live)) return
    if (skipIfMissingIds(t, setup, ["id01","thread_id01"])) return
...
function directSetup(mockres?: any) {
  ...
  if (live) {
    ...
    let idmap: any = env['HUBSPOT_CONVERSATIONS_TEST_CONVERSATIONS_INBOX_MESSAGES_PUBLIC_MESSAGE_ENTID']

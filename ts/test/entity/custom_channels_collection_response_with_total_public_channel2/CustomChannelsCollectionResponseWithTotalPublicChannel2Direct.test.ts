...
describe('CustomChannelsCollectionResponseWithTotalPublicChannel2Direct', async () => {
  ...
  test('direct-list-custom_channels_collection_response_with_total_public_channel2', async (t: any) => {
    if (liveScenariosActive()) { t.skip('Covered by live operation scenarios'); return }
    const setup = directSetup([{ id: 'direct01' }, { id: 'direct02' }])
    if (maybeSkipControl(t, 'direct', 'direct-list-custom_channels_collection_response_with_total_public_channel2', setup.live)) return
    if (skipIfMissingIds(t, setup, ["channel01"])) return
...
function directSetup(mockres?: any) {
  ...
  if (live) {
    ...
    let idmap: any = env['HUBSPOT_CONVERSATIONS_TEST_CUSTOM_CHANNELS_COLLECTION_RESPONSE_WITH_TOTAL_PUBLIC_CHANNEL2_ENTID']

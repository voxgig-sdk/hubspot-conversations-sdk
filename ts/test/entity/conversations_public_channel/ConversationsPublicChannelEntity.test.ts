...
describe('ConversationsPublicChannelEntity', async () => {
  ...
  test('basic', async (t) => {
    ...
    for (const op of ['load']) {
      if (!live && maybeSkipControl(t, 'entityOp', 'conversations_public_channel.' + op, live)) return
    }
    ...
    let conversations_public_channel_ref01_data = Object.values(setup.data.existing.conversations_public_channel)[0] as any

    // LOAD
    const conversations_public_channel_ref01_ent = client.ConversationsPublicChannel()
    const conversations_public_channel_ref01_match_dt0: any = {}

...
describe('ConversationsPublicInboxEntity', async () => {
  ...
  test('basic', async (t) => {
    ...
    for (const op of ['load']) {
      if (!live && maybeSkipControl(t, 'entityOp', 'conversations_public_inbox.' + op, live)) return
    }
    ...
    let conversations_public_inbox_ref01_data = Object.values(setup.data.existing.conversations_public_inbox)[0] as any

    // LOAD
    const conversations_public_inbox_ref01_ent = client.ConversationsPublicInbox()
    const conversations_public_inbox_ref01_match_dt0: any = {}

...
describe('ConversationsCollectionResponseWithTotalPublicInboxDirect', async () => {
  ...
  test('direct-list-conversations_collection_response_with_total_public_inbox', async (t: any) => {
    if (liveScenariosActive()) { t.skip('Covered by live operation scenarios'); return }
    const setup = directSetup([{ id: 'direct01' }, { id: 'direct02' }])
    if (maybeSkipControl(t, 'direct', 'direct-list-conversations_collection_response_with_total_public_inbox', setup.live)) return
...
function directSetup(mockres?: any) {
  ...
  if (live) {
    ...
    let idmap: any = env['HUBSPOT_CONVERSATIONS_TEST_CONVERSATIONS_COLLECTION_RESPONSE_WITH_TOTAL_PUBLIC_INBOX_ENTID']

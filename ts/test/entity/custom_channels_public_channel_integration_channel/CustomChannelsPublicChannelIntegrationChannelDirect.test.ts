...
describe('CustomChannelsPublicChannelIntegrationChannelDirect', async () => {
  ...
  test('direct-load-custom_channels_public_channel_integration_channel', async (t: any) => {
    if (liveScenariosActive()) { t.skip('Covered by live operation scenarios'); return }
    const setup = directSetup({ id: 'direct01' })
    if (maybeSkipControl(t, 'direct', 'direct-load-custom_channels_public_channel_integration_channel', setup.live)) return
    if (skipIfMissingIds(t, setup, ["channel_id01"])) return
...
function directSetup(mockres?: any) {
  ...
  if (live) {
    ...
    let idmap: any = env['HUBSPOT_CONVERSATIONS_TEST_CUSTOM_CHANNELS_PUBLIC_CHANNEL_INTEGRATION_CHANNEL_ENTID']

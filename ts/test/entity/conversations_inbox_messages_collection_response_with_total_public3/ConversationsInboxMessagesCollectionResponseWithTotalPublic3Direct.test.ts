...
describe('ConversationsInboxMessagesCollectionResponseWithTotalPublic3Direct', async () => {
  ...
  test('direct-list-conversations_inbox_messages_collection_response_with_total_public3', async (t: any) => {
    if (liveScenariosActive()) { t.skip('Covered by live operation scenarios'); return }
    const setup = directSetup([{ id: 'direct01' }, { id: 'direct02' }])
    if (maybeSkipControl(t, 'direct', 'direct-list-conversations_inbox_messages_collection_response_with_total_public3', setup.live)) return
...
function directSetup(mockres?: any) {
  ...
  if (live) {
    ...
    let idmap: any = env['HUBSPOT_CONVERSATIONS_TEST_CONVERSATIONS_INBOX_MESSAGES_COLLECTION_RESPONSE_WITH_TOTAL_PUBLIC3_ENTID']

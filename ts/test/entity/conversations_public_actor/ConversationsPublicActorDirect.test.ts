...
describe('ConversationsPublicActorDirect', async () => {
  ...
  test('direct-load-conversations_public_actor', async (t: any) => {
    if (liveScenariosActive()) { t.skip('Covered by live operation scenarios'); return }
    const setup = directSetup({ id: 'direct01' })
    if (maybeSkipControl(t, 'direct', 'direct-load-conversations_public_actor', setup.live)) return
...
function directSetup(mockres?: any) {
  ...
  if (live) {
    ...
    let idmap: any = env['HUBSPOT_CONVERSATIONS_TEST_CONVERSATIONS_PUBLIC_ACTOR_ENTID']

...
describe('ConversationsInboxMessagesPublicMessageContentEntity', async () => {
  ...
  test('basic', async (t) => {
    ...
    for (const op of ['load']) {
      if (!live && maybeSkipControl(t, 'entityOp', 'conversations_inbox_messages_public_message_content.' + op, live)) return
    }
    ...
    let conversations_inbox_messages_public_message_content_ref01_data = Object.values(setup.data.existing.conversations_inbox_messages_public_message_content)[0] as any

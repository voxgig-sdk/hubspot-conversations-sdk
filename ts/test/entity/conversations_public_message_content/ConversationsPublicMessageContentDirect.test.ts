...
describe('ConversationsPublicMessageContentDirect', async () => {
  ...
  test('direct-load-conversations_public_message_content', async (t: any) => {
    if (liveScenariosActive()) { t.skip('Covered by live operation scenarios'); return }
    const setup = directSetup({ id: 'direct01' })
    if (maybeSkipControl(t, 'direct', 'direct-load-conversations_public_message_content', setup.live)) return
    if (skipIfMissingIds(t, setup, ["message_id01","thread_id01"])) return
...
function directSetup(mockres?: any) {
  ...
  if (live) {
    ...
    let idmap: any = env['HUBSPOT_CONVERSATIONS_TEST_CONVERSATIONS_PUBLIC_MESSAGE_CONTENT_ENTID']

...
describe('ConversationsInboxMessagesCollectionResponsePublicThreadEntity', async () => {
  ...
  test('basic', async (t) => {
    ...
    for (const op of ['list']) {
      if (!live && maybeSkipControl(t, 'entityOp', 'conversations_inbox_messages_collection_response_public_thread.' + op, live)) return
    }
    ...
    let conversations_inbox_messages_collection_response_public_thread_ref01_data = Object.values(setup.data.existing.conversations_inbox_messages_collection_response_public_thread)[0] as any
    ...
    const conversations_inbox_messages_collection_response_public_thread_ref01_match: any = {}

    const conversations_inbox_messages_collection_response_public_thread_ref01_list = (await conversations_inbox_messages_collection_response_public_thread_ref01_ent.list(conversations_inbox_messages_collection_response_public_thread_ref01_match)).map((e: any) => e.data())

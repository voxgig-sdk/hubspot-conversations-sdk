...
describe('ConversationsInboxMessagesCollectionResponsePublicThreadDirect', async () => {
  ...
  test('direct-list-conversations_inbox_messages_collection_response_public_thread', async (t: any) => {
    if (liveScenariosActive()) { t.skip('Covered by live operation scenarios'); return }
    const setup = directSetup([{ id: 'direct01' }, { id: 'direct02' }])
    if (maybeSkipControl(t, 'direct', 'direct-list-conversations_inbox_messages_collection_response_public_thread', setup.live)) return
...
function directSetup(mockres?: any) {
  ...
  if (live) {
    ...
    let idmap: any = env['HUBSPOT_CONVERSATIONS_TEST_CONVERSATIONS_INBOX_MESSAGES_COLLECTION_RESPONSE_PUBLIC_THREAD_ENTID']

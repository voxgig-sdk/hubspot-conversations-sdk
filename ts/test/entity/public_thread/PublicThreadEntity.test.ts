...
describe('PublicThreadEntity', async () => {
  ...
  test('basic', async (t) => {
    ...
    for (const op of []) {
      if (!live && maybeSkipControl(t, 'entityOp', 'public_thread.' + op, live)) return
    }
    ...
    let public_thread_ref01_data = Object.values(setup.data.existing.public_thread)[0] as any
...
function basicSetup(extra?: any) {
  ...
  const entityDataFile =
    Path.resolve(__dirname, 
      '../../../../.sdk/test/entity/public_thread/PublicThreadTestData.json')
  ...
  let idmap = transform(
    ['public_thread01','public_thread02','public_thread03'],
    {
      '`$PACK`': ['', {
        '`$KEY`': '`$COPY`',

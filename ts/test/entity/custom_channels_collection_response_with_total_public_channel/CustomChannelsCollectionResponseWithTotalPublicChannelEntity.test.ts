...
describe('CustomChannelsCollectionResponseWithTotalPublicChannelEntity', async () => {
  ...
  test('basic', async (t) => {
    ...
    for (const op of ['list']) {
      if (!live && maybeSkipControl(t, 'entityOp', 'custom_channels_collection_response_with_total_public_channel.' + op, live)) return
    }
    ...
    let custom_channels_collection_response_with_total_public_channel_ref01_data = Object.values(setup.data.existing.custom_channels_collection_response_with_total_public_channel)[0] as any
    ...
    const custom_channels_collection_response_with_total_public_channel_ref01_match: any = {}

    const custom_channels_collection_response_with_total_public_channel_ref01_list = (await custom_channels_collection_response_with_total_public_channel_ref01_ent.list(custom_channels_collection_response_with_total_public_channel_ref01_match)).map((e: any) => e.data())

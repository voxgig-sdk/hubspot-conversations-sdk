...
class CustomChannelsCollectionResponseWithTotalPublicChannel2DirectTest extends TestCase
{
    public function test_direct_list_custom_channels_collection_response_with_total_public_channel2(): void
    {
        $setup = custom_channels_collection_response_with_total_public_channel2_direct_setup([
            ["id" => "direct01"],
            ["id" => "direct02"],
        ]);
        [$_shouldSkip, $_reason] = Runner::is_control_skipped("direct", "direct-list-custom_channels_collection_response_with_total_public_channel2", $setup["live"] ? "live" : "unit");
        if ($_shouldSkip) {
            $this->markTestSkipped($_reason ?? "skipped via sdk-test-control.json");
            return;
        }
        if ($setup["live"]) {
            foreach (["channel01"] as $_liveKey) {

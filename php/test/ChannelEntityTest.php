...
class ChannelEntityTest extends TestCase
{
    ...
    public function test_basic_flow(): void
    {
        ...
        if (!empty($setup["synthetic_only"])) {
            $this->markTestSkipped("live entity test uses synthetic IDs from fixture — set HUBSPOT_CONVERSATIONS_TEST_CHANNEL_ENTID JSON to run live");
            return;
        }
        $client = $setup["client"];

        // Bootstrap entity data from existing test data.
        $channel_ref01_data_raw = Vs::items(Helpers::to_map(
            Vs::getpath($setup["data"], "existing.channel")));
        $channel_ref01_data = null;
        if (count($channel_ref01_data_raw) > 0) {
            $channel_ref01_data = Helpers::to_map($channel_ref01_data_raw[0][1]);
        }
...
function channel_basic_setup($extra)
{
    ...
    $entid_env_raw = getenv("HUBSPOT_CONVERSATIONS_TEST_CHANNEL_ENTID");

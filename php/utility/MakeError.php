...
class HubspotConversationsMakeError
{
    public static function call(?HubspotConversationsContext $ctx, mixed $err): mixed
    {
        if ($ctx === null) {
            require_once __DIR__ . '/../core/Context.php';
            $ctx = new HubspotConversationsContext([], null);
        }
        $op = $ctx->op ?? new HubspotConversationsOperation([]);
        $opname = $op->name;
        if ($opname === '' || $opname === '_') {
            $opname = 'unknown operation';
        }

        $result = $ctx->result ?? new HubspotConversationsResult([]);
        $result->ok = false;
        ...
        $errmsg = ($err instanceof HubspotConversationsError) ? $err->msg : (string)$err;
        ...
        $sdk_err = new HubspotConversationsError('', $msg, $ctx);
        ...
        if ($err instanceof HubspotConversationsError) {

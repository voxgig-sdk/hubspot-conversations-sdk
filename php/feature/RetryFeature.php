...
// HubspotConversations SDK retry feature
...
class HubspotConversationsRetryFeature extends HubspotConversationsBaseFeature
{
    ...
    public function init(HubspotConversationsContext $ctx, array $options): void
    {
        ...
        $utility->fetcher = function (HubspotConversationsContext $ctx2, string $url, array $fetchdef) use ($inner): array {
            return $this->_with_retry($ctx2, $url, $fetchdef, $inner);
        };
    }

    private function _with_retry(HubspotConversationsContext $ctx, string $url, array $fetchdef, callable $inner): array
    {
        $opts = $this->options ?? [];
    ...
    private function _track(HubspotConversationsContext $_ctx, int $attempt, mixed $res, mixed $err, float $wait): void
    {
        $client = $this->client;

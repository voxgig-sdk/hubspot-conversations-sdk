# HubspotConversations SDK test runner
...
class HubspotConversationsTestRunner:
    _env = {}
    ...
    def load_env_local():
        ...
        for line in content.splitlines():
            ...
            HubspotConversationsTestRunner._env[key] = val

    @staticmethod
    def getenv(key):
        val = HubspotConversationsTestRunner._env.get(key)
        if val is not None:
            return val
    ...
    @staticmethod
    def env_override(m):
        live = HubspotConversationsTestRunner.getenv("HUBSPOT_CONVERSATIONS_TEST_LIVE")
        override = HubspotConversationsTestRunner.getenv("HUBSPOT_CONVERSATIONS_TEST_OVERRIDE")

        if live == "TRUE" or override == "TRUE":
            for key in list(m.keys()):
                envval = HubspotConversationsTestRunner.getenv(key)
        ...
        explain = HubspotConversationsTestRunner.getenv("HUBSPOT_CONVERSATIONS_TEST_EXPLAIN")
        if explain is not None and explain != "":
            m["HUBSPOT_CONVERSATIONS_TEST_EXPLAIN"] = explain
    ...
    def load_test_control():
        ...
        """
        if HubspotConversationsTestRunner._test_control is not None:
            return HubspotConversationsTestRunner._test_control
        ctrl_path = os.path.join(os.path.dirname(__file__), "sdk-test-control.json")
        try:
            with open(ctrl_path, "r") as f:
                HubspotConversationsTestRunner._test_control = json.load(f)
        except (FileNotFoundError, IOError, ValueError):
            HubspotConversationsTestRunner._test_control = {
                "version": 1,
                "test": {"skip": {
    ...
    def is_control_skipped(kind, name, mode):
        ...
        ctrl = HubspotConversationsTestRunner.load_test_control()
...
def load_env_local():
    HubspotConversationsTestRunner.load_env_local()


def env_override(m):
    return HubspotConversationsTestRunner.env_override(m)


def entity_data(v):
    return HubspotConversationsTestRunner.entity_data(v)


def entity_list_to_data(lst):
    return HubspotConversationsTestRunner.entity_list_to_data(lst)


def is_control_skipped(kind, name, mode):
    return HubspotConversationsTestRunner.is_control_skipped(kind, name, mode)


def load_test_control():
    return HubspotConversationsTestRunner.load_test_control()


def live_client_options():
    return HubspotConversationsTestRunner.live_client_options()


def live_delay_ms():
    return HubspotConversationsTestRunner.live_delay_ms()

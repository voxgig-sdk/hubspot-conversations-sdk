...
class TestConversationsInboxMessagesCollectionResponsePublicMessageDirect:

    def test_should_direct_list_conversations_inbox_messages_collection_response_public_message(self):
        setup = _conversations_inbox_messages_collection_response_public_message_direct_setup([
            {"id": "direct01"},
            {"id": "direct02"},
        ])
        _skip, _reason = runner.is_control_skipped("direct", "direct-list-conversations_inbox_messages_collection_response_public_message", "live" if setup["live"] else "unit")
        if _skip:
...
def _conversations_inbox_messages_collection_response_public_message_direct_setup(mockres):

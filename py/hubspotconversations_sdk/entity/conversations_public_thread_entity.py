...
class ConversationsPublicThreadEntity:
    ...
    def make(self):
        ...
        for k, v in self._entopts.items():
            opts[k] = v
        return ConversationsPublicThreadEntity(self._client, opts)
    ...
    def data_get(self) -> ConversationsPublicThread:
    ...
    def match_get(self) -> ConversationsPublicThread:
    ...
    def load(self, reqmatch=None, ctrl=None) -> ConversationsPublicThread:
        utility = self._utility
        # reqmatch is optional: an entity with no id-like key loads with no
        # match. Treat None as an empty match so client.ConversationsPublicThread().load()
        # works with no args.
        if reqmatch is None:
            reqmatch = {}
    ...
    def update(self, reqdata: ConversationsPublicThreadUpdateData, ctrl=None) -> ConversationsPublicThread:
        utility = self._utility
        ctx = utility.make_context({
            "opname": "update",
            "ctrl": ctrl,
    ...
    def remove(self, reqmatch=None, ctrl=None) -> ConversationsPublicThread:
        utility = self._utility
        # reqmatch is optional: an entity with no id-like key removes with no
        # match. Treat None as an empty match so client.ConversationsPublicThread().remove()
        # works with no args.
        if reqmatch is None:
            reqmatch = {}
        ctx = utility.make_context({

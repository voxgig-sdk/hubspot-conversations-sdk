...
class PublicThreadEntity:
    ...
    def make(self):
        ...
        for k, v in self._entopts.items():
            opts[k] = v
        return PublicThreadEntity(self._client, opts)
    ...
    def data_get(self) -> PublicThread:
    ...
    def match_get(self) -> PublicThread:
    ...
    def remove(self, reqmatch=None, ctrl=None) -> PublicThread:
        utility = self._utility
        # reqmatch is optional: an entity with no id-like key removes with no
        # match. Treat None as an empty match so client.PublicThread().remove()
        # works with no args.
        if reqmatch is None:
            reqmatch = {}
